...
function Search({ className, onSearch }) {    
    ...
    const onClickSearch = React.useCallback(async () => {
        try {
            ...
            const hasMore = additional_data?.pagination?.more_items_in_collection;
    ...
    useUnmount(() => {
        if (sourceRef.current) {
            sourceRef.current.cancel(`Previous GET /persons/search request has been cancelled`);
        }
    });
    ...
    return (
        <div className={`search${className ? ` ${className}` : ''}`}>
            ...
            <Button type="submit" className="search__filter-btn" onClick={onClickSearch}>Filter</Button>
        </div>
    );
}
