...
function App() {
    ...
    const onAddPerson = React.useCallback((person) => {
        setState({
            ...state,
    ...
    const onSearch = React.useCallback((items, hasMore) => {
        const editedItems = items.map(({ item }) => ({
            ...
            '32dedc4f136ef1c0c8d5c437bbf43a2ef331a525': Array.isArray(item.custom_fields) ? item.custom_fields[0] : '',
            '1dfa3285cdfbcb8dd0b3204d57eb687397542073': Array.isArray(item.custom_fields) ? item.custom_fields[1] : '',
        }));
    ...
    const onDeletePerson = React.useCallback(async (id) => {
        try {
            ...
            if (data.success) {
                ...
                throw new Error('Unsuccessebul delition')
            }
        } catch(error) {
            if (!axios.isCancel(error)) {
    ...
    useUnmount(() => {
        if (getPersonsSourceRef.current) {
            getPersonsSourceRef.current.cancel(`Previous GET /persons request has been cancelled`);
        }
        if (deletePersonSourceRef.current) {
            deletePersonSourceRef.current.cancel(`Previous DEL /person request has been cancelled`);
        }
    });
    ...
    return (
        <div className="App">
            <Header />
            <SubHeader onAddPerson={onAddPerson} onSearch={onSearch} />
            <main>
                <div className="main-container">
                    <PersonsList list={list} hasMore={hasMore} error={error} getPersons={getPersons} onDeletePerson={onDeletePerson} />
                </div>
            </main>
        </div>
    );
